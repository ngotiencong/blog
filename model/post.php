...
class post
{
  ...
  static function update($id,$title,$desc,$content,$slug,$category_id,$account_id,$image = false)
  {
    ...
    if($image){
      ...
      $sql = "UPDATE post SET post_title = '$title', post_desc = '$desc', post_content = '$content', post_image = '$image', post_slug = '$slug',category_id = '$category_id',account_id = '$account_id' WHERE post_id = '$id'";
    }else{
    $sql = "UPDATE post SET post_title = '$title', post_desc = '$desc', post_content = '$content', post_slug = '$slug',category_id = '$category_id',account_id = '$account_id' WHERE post_id = '$id'";
    }
    $dbs = $db->query($sql);
    if($dbs) {
       return true;
  ...
  static function panigation($limit = 5, $page = 1)
  {
    ...
    if(isset($_SESSION['role'])  && $_SESSION['role'] == 1){
      ...
      var_dump($account_id);
      $query      = "SELECT * FROM post WHERE account_id = "." '$account_id' LIMIT ". (($page - 1) * $limit) . ", $limit";
    }else{
      $result = DB::getDB()->query("SELECT count(post_id) AS id FROM post")->fetch_assoc();
      $query      = "SELECT * FROM post  LIMIT " . (($page - 1) * $limit) . ", $limit ";
    }
  ...
  static function getPostByCategory($category_slug)
  {
    $category_id = Category::findBySlug($category_slug)->id;
    $db = DB::getDB();
    $dbs = $db->query("SELECT * FROM post WHERE category_id = '$category_id'");
    if(mysqli_num_rows($dbs)) {
      while ($item = $dbs->fetch_assoc()) {
        if (isset($item['post_id'])) {
          $category_name = Category::find($item['category_id'])->name;
